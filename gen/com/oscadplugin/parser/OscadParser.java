...
public class OscadParser implements PsiParser {
  ...
  protected boolean parse_root_(final IElementType root_, final PsiBuilder builder_, final int level_) {
    return oscadFile(builder_, level_ + 1);
  }
  ...
  /* ********************************************************** */
  // item_*
  static boolean oscadFile(PsiBuilder builder_, int level_) {
    if (!recursion_guard_(builder_, level_, "oscadFile")) return false;
    int offset_ = builder_.getCurrentOffset();
    while (true) {
      if (!item_(builder_, level_ + 1)) break;
      int next_offset_ = builder_.getCurrentOffset();
      if (offset_ == next_offset_) {
        empty_element_parsed_guard_(builder_, offset_, "oscadFile");
        break;
      }
